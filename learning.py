...
# input_dim: an integer specifgenerate_local_predictionsying the number of input neurons
...
def grid_search_model_generator(n_features, n_classes, start_nodes=1, max_nodes=12, max_hlayers=5, debug=False, metrics=['accuracy']):
    ...
    for neurons in range(start_nodes, max_nodes+1):
        for hidden_layer in range(1, max_hlayers+1):
            ...
            model = create_model(n_features, n_classes, neurons,
                                 hidden_layer, name=model_name, metrics=metrics)
...
def perform_grid_search(models, path, dataset_name, X_train, Y_train, X_validation, Y_validation, X_test, Y_test, batch_size, epochs):
    ...
    model_checkpoint = ModelCheckpoint(path + "training/" + dataset_name +
                                       "/model_{epoch:02d}-{val_loss:.2f}.h5", monitor='val_loss',  verbose=0, save_best_only=True,  mode='min')
    ...
    for model in models:
        ...
        history_callback = model.fit(X_train, Y_train, batch_size=batch_size, epochs=epochs, verbose=0, validation_data=(
            X_validation, Y_validation), callbacks=callbacks_list)
        score_test = model.evaluate(X_test, Y_test, verbose=0)
        score_train = model.evaluate(X_train, Y_train)

        print('Test loss:     ', format(
            score_test[0], '.4f'), '\tTrain loss: ', format(score_train[0], '.4f'))
        print('Test accuracy: ', format(
            score_test[1], '.4f'), '\tTrain accu: ', format(score_train[1], '.4f'))
        print('Abs accuracy:  ', format(
            np.abs(score_test[1] - score_train[1]), '.4f'))
        print('Abs loss:      ', format(
            np.abs(score_test[0] - score_train[0]), '.4f'))
        print('\n###########################################################\n')
...
def load_model_history(model_name, path):
    ...
    for line in r.split("\n"):
        ...
        # extract metrics
        metric = line.split(",\"[")[0]
        values_str = line.split(",\"[")[1].replace("]\"", "").split(
...
def generate_save_training_data(dataset_path, X, Y):
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(
        X, Y, test_size=0.3, random_state=515)
    X_validation, X_test, Y_validation, Y_test = train_test_split(
        X_test, Y_test, test_size=0.5, random_state=515)

    np.savetxt(dataset_path.replace(".csv", "") +
               "_Xtrain.csv", X_train, delimiter=",")
    np.savetxt(dataset_path.replace(".csv", "") +
               "_Xtest.csv", X_test, delimiter=",")
    np.savetxt(dataset_path.replace(".csv", "") +
               "_Xvalidation.csv", X_validation, delimiter=",")
    np.savetxt(dataset_path.replace(".csv", "") +
               "_Ytrain.csv", Y_train, delimiter=",")
    np.savetxt(dataset_path.replace(".csv", "") +
               "_Ytest.csv", Y_test, delimiter=",")
    np.savetxt(dataset_path.replace(".csv", "") +
               "_Yvalidation.csv", Y_validation, delimiter=",")
...
def permute_single_features(my_array, samples=300, variance=0.25):
    ...
    for feat in range(0, num_features):
        ...
        for perm in range(0, int(round(samples/num_features, 0))):
            # set the new vector
            my_array[feat] = np.abs(
                np.round(rn.uniform(min_range, max_range), 4))
...
def permute_single_features_circle(my_array, samples=300, variance=0.25):
    ...
    for perm in range(0, int(round(samples/num_features, 0))):
        ...
        for feat in range(0, num_features):
            ...
            # set the new vector
            temp1.append(check_input(
                feature_val + np.round(np.random.uniform(0, variance), 4)*math.cos(theta)))
            temp2.append(check_input(
                feature_val + np.round(np.random.uniform(0, variance), 4)*math.sin(theta)))
    ...
    for col in df.columns.values:
        ...
        if col != class_var:
            l.append(pd.DataFrame(
                pd.qcut(df[col], r, duplicates='drop', precision=2), columns=[col]))
        else:
...
def save_discretized_dataframe(indx, df_model, model_type, perm_type, bins, dataset_name, path, class_var):
    ...
    #TODO: why the saved file is tuple?
    file_path = path + dataset_name + "/" + str(indx) + "/" + re.sub(
        r"\.\w+", "", dataset_name) + "_" + model_type + "_INDX_" + str(indx) + "_" + perm_type + ".csv"
...
def generate_permutations(instance, labels_lst, feature_names, class_var, encoder, scaler, model, samples=300, variance=0.25):
    ...
    # permute features
    permutations = permute_single_features(
        local_datapoint, samples=samples, variance=variance)
    ...
    labelled_predictions = [
        labels_lst[int(predictions[indx][0])] for indx in range(0, len(predictions))]

    # add all this information to a single dataframe
    df_local_permutations = pd.DataFrame(
        permutations_orig, columns=feature_names)
...
def generate_BN_explanations(instance, label_lst, feature_names, class_var, encoder, scaler, model, path, dataset_name):
    ...
    df = generate_permutations(
        instance, label_lst, feature_names, class_var, encoder, scaler, model)

    # discretize data (No need for cat data.)
    df_discr = discretize_dataframe(df, class_var, num_bins=4)

    # save discretised dataframe (for debugging and reproduceability purposes) # (TODO: find this saved file)
    path_to_permutations = path + "feature_permutations/" + \
        dataset_name.replace(".csv", "") + "/" + \
        prediction_type + "/" + str(indx) + ".csv"
    ...
    # save to file
    path_to_explanation = path + "explanations/" + \
        dataset_name.replace(".csv", "") + "/BN/" + prediction_type + "/"
...
def generate_BN_explanationsMB(instance, label_lst, feature_names, class_var, encoder, scaler, model, path, dataset_name, variance=0.1, algorithm="Hill Climbing"):
    ...
    # generate permutations
    df = generate_permutations(instance, label_lst, feature_names,
                               class_var, encoder, scaler, model, variance=variance)
    ...
    bn, infoBN, essencGraph = learnBN(path_to_permutations.replace(
        ".csv", "_norm.csv"), algorithm=algorithm)
...
def normalise_line(my_str, class_label):
    ...
    for token in tokens:
        ...
        if((len(token[indxs[1]+1: -1]) >= 4) & (len(token[indxs[0]+1: indx_comma-2]) >= 4)):
            token_temp = token[0:indxs[0]] + "." + token[indxs[0] + 1: indxs[0]+5] + \
                ", " + token[indx_comma:indxs[1]] + \
                token[indxs[1]: indxs[1]+5] + "]"

        if((len(token[indxs[1]+1: -1]) < 4) & (len(token[indxs[0]+1: indx_comma-2]) >= 4)):
            extra = "0"*(np.abs(len(token[indxs[1]+1: -1]) - 4))
            token_temp = token[0:indxs[0]] + "." + token[indxs[0] + 1: indxs[0]+5] + \
                ", " + token[indx_comma:indxs[1]] + \
                token[indxs[1]: -1] + extra + "]"

        if((len(token[indxs[1]+1: -1]) >= 4) & (len(token[indxs[0]+1: indx_comma-2]) < 4)):
            extra = "0"*(np.abs(len(token[indxs[0]+1: indx_comma-2]) - 4))
            token_temp = token[0:indxs[0]] + "." + extra + ", " + \
                token[indx_comma:indxs[1]] + token[indxs[1]: -1] + extra + "]"

        if((len(token[indxs[1]+1: -1]) < 4) & (len(token[indxs[0]+1: indx_comma-2]) < 4)):
            extra2 = "0"*(np.abs(len(token[indxs[1]+1: -1]) - 4))
            extra1 = "0"*(np.abs(len(token[indxs[0]+1: -1]) - 4))
            token_temp = token[0:indxs[0]] + "." + extra1 + ", " + \
                token[indx_comma:indxs[1]] + token[indxs[1]: -1] + extra2 + "]"
